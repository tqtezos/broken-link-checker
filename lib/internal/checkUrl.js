...
function checkUrl(link, baseUrl, cache, options, retry)
{
	...
	.then( function(response)
	{
		...
		if (response.statusCode === 404 && options.requestMethod==="head" && options.retry404Head===true && retry!==404) {
			// Retry erroneous 404 because of trailing slash
			link.url.resolved = link.url.resolved.replace(/\/$/,'');
			link.url.original = link.url.original.replace(/\/$/,'');
...
{
	const ignoreRedirect = (redirectUrl, responseUrl) => {
		// Don't log redirects to add trailing slashes, /index, /index.htm /index.html
		const re = new RegExp(`${redirectUrl}/?(index)?(\.html?)?$`)
	...
	{	
		if (response.statusCode !== 200)
		{
			...
			!(ignoreRedirect(response.redirects[0].url, response.url))) {
			link.warning = true;
			link.warningReason = `REDIRECTED to ${response.url}`;
		}
		else
		{
			link.broken = false;
