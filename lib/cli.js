...
function cli()
{
	...
	this.nopter.config(
	{
		...
		options:
		{
			...
			// "exclude-warnings": {
			// 	rename: "excludedWarnings",
			// 	info: "A keyword/glob to match links against. Can be used multiple times. Will still fail if broken, but not on warning",
			// 	type: [String, Array],
			// 	default: defaultOptions.excludedWarnings
			// },
			"exclude-external":
			{
			...
			{
				rename: "failWarnings", 
				info: "Fail tests on warnings",
				type: Boolean,
				default: defaultOptions.failWarnings
			}
		},
		aliases: ["input"]
	});
...
cli.prototype.input = function(args, showArgs)
{
	...
	else if (args.input != null)
	{
		...
		run(args.input,
		{
			excludedKeywords:       args.excludedKeywords || defaultOptions.excludedKeywords,
			// excludedWarnings: 		args.excludedWarnings || defaultOptions.excludedWarnings,
		...
		},
		{
			...
			failWarnings: 		  args.failWarnings===true
		});
	}
	else
	{
...
function logMetrics(brokenLinks, warnings, excludedLinks, totalLinks, duration, preBreak, failWarnings, exit)
{	
	var getChalk = () => {
		var blStr = brokenLinks + " broken; ";
		var wStr = warnings + " warnings"
		if (brokenLinks && warnings) {
			return chalk["red"](blStr + wStr);
		}
		if (brokenLinks) {
			return chalk["red"](blStr);
		}
		if (warnings) {
			return chalk["yellow"](wStr);
		}
		return chalk["green"]("All links valid")
	}
	...
	if (exit === true)
	{
		process.exit(brokenLinks === 0 && (warnings === 0 || failWarnings === false) ? 0 : 1);
	}
}
...
*/
function logMetrics_delayed(brokenLinks, warnings, excludedLinks, totalLinks, duration, preBreak, failWarnings, exit)
{
	setImmediate( function()
	{
		logMetrics(brokenLinks, warnings, excludedLinks, totalLinks, duration, preBreak, failWarnings, exit);
	});
}
...
function logPage(data, pageUrl)
{
	...
	output += chalk.white("Getting links from: ") + chalk.cyan(pageUrl);
...
function logResult(result, finalResult)
{
	...
	if (result.__cli_excluded !== true)
	{
		...
		{
			output += chalk.cyan( result.url.resolved );
		}
		else
		{
			// Excluded scheme
			output += chalk.cyan( result.url.original );
		}
		...
		{
			output += chalk.gray(" ("+ result.warningReason +")");
		}
		else if (result.excluded === true)
		{
	...
	handlers =  
	{
		...
		page: function(error, pageUrl)
		{
			...
			else
			{
				...
				logMetrics_delayed(data.page.brokenLinks, data.page.warnings, data.page.excludedLinks, data.page.totalLinks);
			}
		},
		end: function()
		{
			...
			{
				process.exit(data.page.done===true && data.total.brokenLinks===0 && data.total.warnings === 0 ? 0 : 1);
			}
			else if (data.total.pages > 1)
			{
				logMetrics_delayed(data.total.brokenLinks, data.total.warnings, data.total.excludedLinks, data.total.links, Date.now()-data.total.startTime, true, logOptions.failWarnings, true);
			}
		}
	};
