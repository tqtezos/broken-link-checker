...
function logMetrics(brokenLinks, warnings, excludedLinks, totalLinks, duration, preBreak, exit)
{	
	var getChalk = () => {
	...
	if (exit === true)
	{
		process.exit(brokenLinks === 0 && warnings === 0 ? 0 : 1);
	}
}
...
*/
function logMetrics_delayed(brokenLinks, warnings, excludedLinks, totalLinks, duration, preBreak, exit)
{
	setImmediate( function()
	{
		logMetrics(brokenLinks, warnings, excludedLinks, totalLinks, duration, preBreak, exit);
	});
}
...
function run(url, checkerOptions, logOptions)
{
	...
	handlers =  
	{
		...
		end: function()
		{
			...
			{
				logMetrics_delayed(data.total.brokenLinks, data.total.warnings, data.total.excludedLinks, data.total.links, Date.now()-data.total.startTime, true, true);
			}
		}
	};
